...
void dibujarTablero(Tablero tablero) {
    ...
    for (f=4;f<FILAS;f++) {
        for (c=0;c<COLUMNAS;c++) {
            ...
            if(tablero[f][c]!=15) { //OPTIMIZACION CONSIDERO PORQUE SINO ESTARIA DIBUJANDO TODO EL TIEMPO
                ...
                /* Calculamos la esquina superior izquierda del bloque
                   ...
                   yBase: Margen superior + ((fila - 2) * tamaño del bloque)
                   Restamos 2 a la fila para que la fila 2 de la matriz sea la 0 en pantalla
                */
                int xBase=MARGEN_X+(c*TAM_BLOQUE);
                int yBase=MARGEN_Y+((f-2)*TAM_BLOQUE);
...
/*void dibujarPiezasenTableroEstaticas(Tablero tablero){

}*/

void dibujar_Pieza_Caida(Tablero tablero,Pieza pieza){

}

/*void dibujar_Entorno(Tablero tablero){
    int f,c;
